...
if ($_SESSION["stubia_userperfil"] ==="Profesor" ) {
    exit("No tiene permisos para visusalizar esta página");
}
...
if(isset($_POST['btn_reservar']) && !empty($_POST['hid_puesto']) && !empty($_POST['hid_fecha'])){    
    ...
    if (!$consulta) { 
        ...
        $resultado="Lo sentimos, no se puede reservar más de un puesto en la misma fecha y hora (".date_format(date_create($fecha),'d/m/Y H:00').").";
        $color_res="red";
            ...
            if ($consulta->num_rows>=MAX_RESERVAS_DIA) {
                ...
                if($insertar){
                    ...
                    if ($consulta) { //Hemos guardado la reserva en BD, enviamos el email:
                        ...
                        $mensaje.= "<ul><li>Fecha y hora: ".date_format(date_create($fecha),'d/m/Y H:00')." (1 hora de duraci&oacute;n).</li>";        
...
$sql = "SELECT r.id, r.fecha, f.inicio, p.puesto FROM reservas r "
        ...
        . "INNER JOIN master_franjas_horarias f ON r.id_franja_horaria=f.id AND f.activo=1 "    
        . "WHERE p.id_aula=3 AND r.id_usuario=".$_SESSION["stubia_userid"]." AND r.activo=1 "
        . "AND YEAR(r.fecha)>=YEAR(SYSDATE()) AND MONTH(r.fecha)>=MONTH(SYSDATE()) "
        . "AND (DAY(r.fecha)>DAY(SYSDATE()) OR (DAY(r.fecha)=DAY(SYSDATE()) AND f.inicio>=HOUR(SYSDATE()))) "
